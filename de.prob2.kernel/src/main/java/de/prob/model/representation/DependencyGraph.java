...
/**
 * A simple graph implementation intended to display the relationships between the components
 * in a model. The nodes in the graph are represented as the string element name of the component
 * and the edges are the type of relationship as defined by {@link ERefType}. It is a multigraph,
 * which means that there can be multiple edges between two nodes.
 * 
 * @author joy
 *
 */
public class DependencyGraph {
	...
	private class Edge {
		...
		public boolean equals(Object that) {
			...
			if (that instanceof Edge) {
				return this.getFrom().equals(((Edge) that).getFrom()) &&
						this.getTo().equals(((Edge) that).getTo()) &&
						this.getRelationship().equals(((Edge) that).getRelationship());
			}
			return false;
		}
	...
	/**
	 * @param element element to be added to the graph it is has not already been added;
	 */
	public void addVertex(String element) {
	...
	/**
	 * @param element to be found in the graph
	 ...
	 * @param relationship relationship between the two elements
	 */
	public void addEdge(String from, String to, ERefType relationship) {
	...
	public String toString() {
		...
		for (Entry<String, Node> entry : graph.entrySet()) {
			...
			for (Edge edge : outEdges) {
				s.add(edge.getRelationship().toString() + "=(" + edge.getTo().getElementName() + "," + edge.getFrom().getElementName() + ")");
			}
		}
