...
class BMotionUtil {
    ...
    def static void evaluateGroovy(ScriptEngine engine, String absoluteTemplatePath, Map<String, String> parameters,
                                   BMotion bms) throws GroovyRuntimeException, ScriptException {
    ...
    def static AbstractModel loadModel(Api api, AnimationSelector animations, String modelPath,
                                       String absoluteTemplatePath) {
        def AbstractModel model = null
        if (modelPath != null) {
            def formalism = BMotionUtil.getFormalism(modelPath)
            def path = FilenameUtils.
                    separatorsToSystem(BMotionUtil.getTemplateFolder(absoluteTemplatePath) + File.separator + modelPath)
    ...
    def static ITool loadTool(String sessionId, String toolId, String modelPath, AnimationSelector animations,
                              ToolRegistry toolRegistry, Api api, String absoluteTemplatePath,
                              Class<? extends AbstractBMotionStudioServlet> claz) {
    ...
    def static String getFormalism(final String machinePath) {
        ...
        if (machinePath.endsWith(".csp")) {
            return "csp";
        } else if (machinePath.endsWith(".buc") || machinePath.endsWith(".bcc") || machinePath.
                endsWith(".bum") || machinePath.endsWith(".bcm")) {
            return "eventb";
