...
public class CGGConstructionTest extends Specification {
	...
	def print(graph) {
		...
		graph.outgoingEdges.each { Statement, Set<Edge> edges ->
			edges.each { Edge e ->
				println e.from.toString() + e.toString() + e.to.toString()
			}
		}
	}
