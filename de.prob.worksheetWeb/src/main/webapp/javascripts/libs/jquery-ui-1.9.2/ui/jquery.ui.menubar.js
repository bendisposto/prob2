...
(function($) {
	...
	// TODO when mixing clicking menus and keyboard navigation, focus handling
	// is broken
	...
	$.widget("ui.menubar", {
		...
		_create : function() {
			...
			this.items.siblings(this.options.menuElement).menu({
				...
				select : function(event, ui) {
					...
					that._close();
					// TODO what is this targetting? there's probably a better
					// way to access it
			...
			this.items.each(function() {
				var input = $(this),
				// TODO menu var is only used on two places, doesn't quite
				// justify the .each
				...
				if (menu.length) {
					...
					input.bind("click.menubar focus.menubar mouseenter.menubar", function(event) {
						...
						event.preventDefault();
						// TODO can we simplify or extractthis check? especially
					...
					// TODO review if these options (menuIcon and buttons) are a
					// good choice, maybe they can be merged
				...
				input.addClass("ui-button ui-widget ui-button-text-only ui-menubar-link").attr("role", "menuitem").wrapInner("<span class='ui-button-text'></span>");
		...
		_destroy : function() {
			...
			this.items.unbind(".menubar").removeClass("ui-button ui-widget ui-button-text-only ui-menubar-link ui-state-default").removeAttr("role").removeAttr("aria-haspopup")
			// TODO unwrap?
		...
		_open : function(event, menu) {
			...
			// TODO refactor, almost the same as _close above, but don't remove
			// tabIndex
			if (this.active) {
				this.active.menu("collapseAll").hide().attr({
					"aria-hidden" : "true",
			...
			// set tabIndex -1 to have the button skipped on shift-tab when menu
			// is open (it gets focus)
			var button = menu.prev().addClass("ui-state-active").attr("tabIndex", -1);
			this.active = menu.show().position($.extend({
				of : button
			}, this.options.position)).removeAttr("aria-hidden").attr("aria-expanded", "true").menu("focus", event, menu.children(".ui-menu-item").first())
		...
		next : function(event) {
			if (this.open && this.active.data("menu").active.has(".ui-menu").length) {
				// Track number of open submenus and prevent moving to next
				// menubar item
				this.openSubmenus++;
				return;
			}
		...
		previous : function(event) {
			if (this.open && this.openSubmenus) {
				// Track number of open submenus and prevent moving to previous
				// menubar item
				this.openSubmenus--;
				return;
			}
