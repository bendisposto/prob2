...
/**
 * Checks CSP assertions from the CSP specification, as well as 
 * new on-the-fly defined CSP assertions (assertions which are not in the CSP specification)
 * 
 * @author dobrikov
 * 
 */
...
m = api.csp_load("/Users/ivo/ProB/probprivate/public_examples/CSP/other/Ivo/Deterministic1.csp")
...
s.execute(command)
command.getResults() // getting the list of results
command.getResultTraces() // getting the list of possible counter example traces
...
public class CSPAssertionsCommand extends AbstractCommand {
	...
	private List<ListPrologTerm> resultTraces = new ArrayList<ListPrologTerm>();
	...
	public void processResult(
		...
		ListPrologTerm prologTermResultTraces = BindingGenerator.getList(bindings
				.get(RESULT_TRACES_VARIABLE));
